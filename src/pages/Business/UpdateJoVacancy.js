...
export default function UpdateJobVacancy() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      ...
      // setJobVacancy({
      //   ...jobVacancy,
      //   registration_end_date: getFullDate(jobVacancy.registration_end_date),
      // })
      setLoading(false);
    };
    getData();
  ...
  const handleSubmit = async () => {
    ...
    const response = await updateJobVacancy(idJob, jobVacancy);
                    ...
                    <Dialog.Panel className="w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                      <div>
                        ...
                        <p className="mx-auto w-full text-center text-[#7E8299] lg:w-[400px]">
                          Lowongan kerja berhasil diupdate!
                        </p>
                      </div>
                    </Dialog.Panel>
                  </Transition.Child>
...
function InputList({ label, data, name, onChange }) {
  ...
  const [list, setList] = useState([...data[name]]);
  const addList = () => {
    if (input !== '') {
      setList([...list, input]);
      onChange({
