export default function Search({search, handleSearchChange, handleSearch, handleReset}) {
  return (
    <div className="flex flex-col justify-between gap-3 rounded-md bg-white p-3 text-xs shadow-mine sm:flex-row sm:gap-7 sm:p-7 lg:text-sm">
      ...
      />
      <button onClick={search.isSearching ? handleReset : handleSearch} className="primary-btn flex w-fit items-center justify-center gap-2 rounded-md px-5 py-2">
        <svg
          width="15"
          height="15"
          viewBox="0 0 15 15"
        ...
        <p>{search.isSearching ? 'Reset' : 'Filter'}</p>
      </button>
    </div>
  )
}