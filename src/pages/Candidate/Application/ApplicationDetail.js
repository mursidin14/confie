...
export default function ApplicationDetail() {
  ...
  let idle_style = 'border-[#7E8299] bg-[#F5F8FA] text-[#7E8299]'
  let steps = [
    {
      number: 1,
  ...
  return (
    <Layout userId={'1'} PageName={'Lamaran Saya'}>
      ...
      <section className="overflow-auto my-6 flex w-full justify-between items-center gap-2 rounded-md bg-white p-8 text-left text-[#3F4254] shadow-mine">
        {steps.map((step) => (
          <>
            <p
              className={`rounded-md border p-3 flex-1 text-center text-xs ${step.number <= status.step ? accept_style : idle_style } ${status.rejected && step.number <= status.step ? refuse_style : ''}`}
            >
              {step.title}
            </p>
            <svg className='last:hidden' width="51" height="12" viewBox="0 0 51 12" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M50.5303 6.53033C50.8232 6.23744 50.8232 5.76256 50.5303 5.46967L45.7574 0.696699C45.4645 0.403806 44.9896 0.403806 44.6967 0.696699C44.4038 0.989593 44.4038 1.46447 44.6967 1.75736L48.9393 6L44.6967 10.2426C44.4038 10.5355 44.4038 11.0104 44.6967 11.3033C44.9896 11.5962 45.4645 11.5962 45.7574 11.3033L50.5303 6.53033ZM0 6.75H50V5.25H0V6.75Z" fill="#E5E5E5"/>
</svg>
