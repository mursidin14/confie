import React, { useNavigate, useState, useEffect, Fragment, useRef } from 'react';
  ...
  useEffect(() => {
	...
	if (url.current.split('?')[1]) {
		...
		try {
			...
			setOpenModal(true)
			setError(true)
			setError_msg(error.response.data.meta.message)
		}
		setPage(2);
		setDataAccount({
			...dataAccount,
		...
		<>  
			{ openModal && (
				<>
					<ModalError
						error={error}
						error_msg="The link has been invalid or expired"
