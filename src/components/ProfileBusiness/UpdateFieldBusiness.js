...
function InputTag({ addTags, people, role }) {
  ...
  async function handleClick(e) {
    const id = people.find((person) => person.name === e.target.innerText).name;
    ...
    <Combobox value={selected} onChange={setSelected}>
      <div className="mt-1 grow">
        <Combobox.Input
          ...
          onKeyUp={(e) => {
            if (e.key === 'Enter') {
              addTags(selected.name, selected.name, nameSkill);
            }
          }}
          ...
          <Combobox.Options className="absolute mt-1 h-[100px] max-h-60 w-fit overflow-auto rounded-md bg-white text-left shadow-lg sm:max-w-[400px]">
            {filteredPeople.length === 0 && query !== '' ? (
              <div className="relative z-10 cursor-default select-none py-2 px-4 text-gray-700">
                Press enter to add arbitrary field
              </div>
            ) : (
...
export default function UpdateFieldBusiness({skills, data, onChange}) {
  ...
  const removeTags = (indexToRemove) => {
    ...
    onChange({
      ...data,
      company_type: [...data.company_type.filter((_, index) => index !== indexToRemove)],
    });
  };
  const addTags = (value, id, value2) => {
    console.log('v1',value, 'v2', value2);
    if (value !== '' && value !== undefined) {
      if (tags.map((tag) => tag.name).includes(value)) {
        ...
        onChange({
          ...data,
          company_type: [...data.company_type, value2],
        });
        return;
      }
      setTags([
      ...
      onChange({
        ...data,
        company_type: [...data.company_type, id],
      });
      return;
    }
    if (value2) {
