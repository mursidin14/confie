    ...
    <div className="lg:relative">
      <div className="mt-4 rounded-md bg-white pt-7 pb-2  text-left shadow-mine ">
        ...
        <div className="flex justify-center">
          {certification.length > 3 ? <Pagination length={certification.length} pagination={pagination} setPagination={setPagination} /> : null}
        </div>
      </div>
    </div>
  );
    ...
    <table className="w-full min-w-[700px] table-fixed text-center text-xs sm:text-base">
      ...
      {!items.length > 0 && (
        ...
        {items.map((item, index) => (
          <tr
            className="mt-3 sm:h-20 h-10 border-b-2 border-gray-300/50 text-sm text-[#7E8299]"
            key={index}
          >
